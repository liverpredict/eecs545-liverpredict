...
def plot_training_curve(history, args):
	key = args.y_train_file.replace('data_no_imputation/','').replace('.csv','')
	figure_name = 'results_no_imputation/training_curve/acc_' + key + '_' + str(args.num_hidden_layers) + '_' + str(args.epochs) + '.eps'
	plt.figure()
	...
	figure_name = 'results_no_imputation/training_curve/loss_' + key + '_' + str(args.num_hidden_layers) + '_' + str(args.epochs) + '.eps'
	plt.figure()
...
def get_data(args):
	...
	# normalization of the data_no_imputation
	sc = StandardScaler()
	x = sc.fit_transform(x)
	x_test = sc.fit_transform(x_test)

	# create one hot encoded data_no_imputation
	ohe = OneHotEncoder()
	y = ohe.fit_transform(y).toarray()
	y_test = ohe.fit_transform(y_test).toarray()

	# split the data_no_imputation for training and validation
...
def evaluate(x_test, y_test, model, args):
	...
	key = args.y_train_file.replace('data_no_imputation/','').replace('.csv','')
	file_name = 'results_no_imputation/' + key + '_' + str(args.num_hidden_layers) + '_' + str(args.epochs) + '.txt'
	f = open(file_name, "w")
	f.write(str(acc) + '\n\n')
	...
	plt.figure()
	figure_name = 'results_no_imputation/roc_curve/' + key + '_' + str(args.num_hidden_layers) + '_' + str(args.epochs) + '.eps'
...
if __name__ == "__main__":
	...
	parser.add_argument('--x_train_file', action = "store", dest = "x_train_file", default = "data_no_imputation/x_train.csv", type = str)
	parser.add_argument('--x_test_file', action = "store", dest = "x_test_file", default = "data_no_imputation/x_test.csv", type = str)
	parser.add_argument('--y_train_file', action = "store", dest = "y_train_file", default = "data_no_imputation/y_180_train.csv", type = str)
	parser.add_argument('--y_test_file', action = "store", dest = "y_test_file", default = "data_no_imputation/y_180_test.csv", type = str)
